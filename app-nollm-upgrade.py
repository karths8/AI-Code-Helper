...
def code_helper():
    ...
    if question_num=='finish':
        question_set = [' Q'+str(idx) for idx in range(1, num_of_questions+1)]
        for question_num in question_set:
            ...
            if q_key in questions_cache and cached_code==code:
                ...
                if error=="All tests Passed!":
                    ...
                    model_resp = "This program has errors, please check!"
                    questions_cache[q_key] = {"question":question_str, "code":code, "suggestion": model_resp, 'error':error}
                    generated_content = model_resp
        get_student_results()
        total_score = sum([results[i]['score'] for i in results])
        return render_template('index_nollm.html', results=results, student_code=None, question_num=None, question_str=None, num_of_questions=num_of_questions, error=None, total_score=total_score)
    return render_template('index_nollm.html', student_code=code, question_num=num, question_str=question_str, num_of_questions=num_of_questions, error=error, total_score=total_score, results=None)
